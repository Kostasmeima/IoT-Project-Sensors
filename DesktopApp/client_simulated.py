# File: client_simulated.py
# Description: Example code to show simulated connection using Wi-Fi as a client when
...
class MainApp(IoTApp):
    ...
    def init(self):
        ...
        #sckt.connect(("###.###.###.###", 2350))  # Note: passed in as 2-tuple here
        sckt.connect(("127.0.0.1", 2350))
        
        # Wait until a message is received from the server, note: the buffer size is set attempt
        # 16 bytes so only this many will be received, if more are sent then they will be
        # ignored, if you need to send a larger message from the server then make sure the buffer
        # is set with a suitable size here
        msg_as_bytes = sckt.recv(16)  # This blocks until message is received 

        # Display message sent to the client from the server, note: this will be as a byte
        # stream so it needs converting (decoding) to ASCII characters as a displayable
        # string
...
def main():
    ...
    # property values:-
    #
    #   name: "Client Sim'ed", this should be a maximum of 14 characters else it is truncated
    ...
    #
    app = MainApp(name="Client Sim'ed", has_oled_board=True, finish_button="C", start_verbose=True)

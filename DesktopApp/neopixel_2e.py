# File: neopixel_2e.py
# Description: Sample code for Week 3 Task 2e 
...
def main():
    ...
    # Randomly colour a NeoPixel in the matrix a random colour once every 0.1
    # seconds for 100 times
    for _ in range(100):
        # Randomly select matrix position
        matrix_pos = randint(0, 31)
        
        # Randomly select colour
        rchannel = randint(0, 10)
        gchannel = randint(0, 10)
        bchannel = randint(0, 10)
        colour = (rchannel, gchannel, bchannel)

        # Colour matrix position
        npm[matrix_pos] = colour
        npm.write()

        # Show for 0.1 seconds
        sleep(0.1)

        # Clear matrix position colour    
        npm[matrix_pos] = (0, 0, 0)
        npm.write()

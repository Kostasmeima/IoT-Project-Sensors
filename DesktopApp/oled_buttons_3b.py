# File: oled_buttons_3b.py
# Description: Sample code for Week 3 Task 3b 
...
direction = 0  # Use this to set the direction the vertical line should
               # animate: 1 is left to right, -1 is right to left, 0 is
               # stopped

def display_line(col, colour):
    npm[col] = colour
    npm[col + 8] = colour
    npm[col + 16] = colour
    npm[col + 24] = colour
...
def main():
    ...
    # Wire-up the function button_c_handler() as the interrupt handler for Button C
    ...
    # Use this variable to control where the vertical line should display on the
    # matrix
    col = 3
    # Run the program for ever
    while True:
        # Remove the existing vertical line
        display_line(col, (0, 0, 0))
        
        # Move the vertical line according to the direction of travel (or not moved
        # if direction is 0)
        col += direction
        
        # Make sure vertical line stops when against left hand or right hand side
        if col < 0:
            col = 0
            direction = 0  # Stopped
        elif col > 7:
        ...
        # Display the vertical line as red if moving left to right, green if moving
        # right to left and blue if stopped
        colour = (0, 0, 10)  # Assumed stopped
        if direction == 1:
            colour = (10, 0, 0)  # Red
        elif direction == -1:
            colour = (0, 10, 0)  # Green
            
        display_line(col, colour)
